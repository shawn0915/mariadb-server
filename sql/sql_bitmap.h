...
{
public:
  uint32 buffer[width / 32];
public:
  Bitmap():buffer()
  {
    compile_time_assert(width % 32 == 0);
  }
  ...
  void intersect_and_pad(ulonglong map2buff, int pad)
  {
    ...
    if (array_elements(buffer) > 2)
      memset((char*)buffer + 8, pad, sizeof(buffer) - 8);
  }
  ...
  {
    intersect_and_pad(map2buff, (map2buff & (1ULL << 63)) ? 0xff : 0);
  }
  ...
  uchar last_byte_mask(uint bits) const
  {
...
#else
typedef Bitmap<((MAX_INDEXES+31)/32*32)> key_map; /* Used for finding keys */
#endif
